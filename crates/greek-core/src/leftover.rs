...
use greek_common::{
    LeftoverAnalyzer, InstalledApp, LeftoverArtifact, ArtifactType, SafetyLevel, AnalysisError, Result,
};
...
impl LeftoverAnalyzer for FileSystemLeftoverAnalyzer {
    ...
    async fn analyze(
        ...
        let publisher_lower = app.publisher.as_ref()
        ...
        for directory in &self.scan_directories {
            ...
            let found = self.scan_directory_for_app(directory, &app_name_lower, &publisher_lower).await?;
...
impl FileSystemLeftoverAnalyzer {
    async fn scan_directory_for_app(
            ...
            .filter_map(|e| e.ok())
        {
            ...
            if path_str.contains(app_name) || (!publisher.is_empty() && path_str.contains(publisher)) {
                ...
                if let Some(dir_name) = path.file_name() {
                    ...
                    if let Ok(metadata) = std::fs::metadata(path) {
                        if let Ok(modified) = metadata.modified() {
                            let datetime: chrono::DateTime<chrono::Utc> = chrono::DateTime::from(modified);
                            let age = chrono::Utc::now().signed_duration_since(datetime);
                            
                            // If older than 30 days, might be orphaned
                            if age.num_days() > 30 {
                                let mut artifact = LeftoverArtifact::new(ArtifactType::Directory, path.to_path_buf());
                                artifact.confidence = 0.3; // Low confidence for orphan detection
                                artifact.safety_level = SafetyLevel::Caution;
                                artifact.description = format!("Potential orphan directory: {}", dir_name_str);
                                artifact.size_bytes = Some(metadata.len());
...
impl LeftoverAnalyzerManager {
    ...
    async fn analyze_system(&self) -> Result<Vec<LeftoverArtifact>> {
        ...
        for analyzer in &self.analyzers {
            let artifacts = analyzer.analyze_system().await?;
...
mod tests {
    ...
    async fn test_filesystem_analyzer() {
        ...
        let app = InstalledApp::new("TestApp".to_string(), InstallSource::Registry {
    ...
    fn test_confidence_scoring() {
        ...
        let artifact = LeftoverArtifact::new(ArtifactType::Directory, PathBuf::from("C:\\Users\\Test\\TestApp"));
