...
use greek_common::{InstalledApp, InstallSource, BrowserType, Result, GreekError};
...
impl BrowserExtensionScanner {
    ...
    fn get_extension_dir(&self, browser: &BrowserType) -> Option<PathBuf> {
        ...
        match browser {
            BrowserType::Chrome => {
                ...
                {
                    Some(PathBuf::from(std::env::var("LOCALAPPDATA").ok()?)
                        .join("Google").join("Chrome").join("User Data").join("Default").join("Extensions"))
                }
                #[cfg(target_os = "linux")]
                {
                    Some(PathBuf::from(&home).join(".config").join("google-chrome").join("Default").join("Extensions"))
                }
                #[cfg(target_os = "macos")]
                {
                    Some(PathBuf::from(&home).join("Library").join("Application Support").join("Google").join("Chrome").join("Default").join("Extensions"))
                }
            ...
            BrowserType::Firefox => {
                #[cfg(target_os = "windows")]
                {
                    Some(PathBuf::from(std::env::var("APPDATA").ok()?)
                ...
                #[cfg(target_os = "macos")]
                {
                    Some(PathBuf::from(&home).join("Library").join("Application Support").join("Firefox").join("Profiles"))
                }
                #[cfg(not(any(target_os = "windows", target_os = "linux", target_os = "macos")))]
                {
                    None
            ...
            BrowserType::Edge => {
                ...
                {
                    Some(PathBuf::from(std::env::var("LOCALAPPDATA").ok()?)
                        .join("Microsoft").join("Edge").join("User Data").join("Default").join("Extensions"))
                }
                #[cfg(target_os = "linux")]
                {
                    Some(PathBuf::from(&home).join(".config").join("microsoft-edge").join("Default").join("Extensions"))
                }
                #[cfg(target_os = "macos")]
                {
                    Some(PathBuf::from(&home).join("Library").join("Application Support").join("Microsoft Edge").join("Default").join("Extensions"))
                }
                #[cfg(not(any(target_os = "windows", target_os = "linux", target_os = "macos")))]
                {
                    None
            ...
            BrowserType::Opera => {
                ...
                {
                    Some(PathBuf::from(std::env::var("APPDATA").ok()?)
                        .join("Opera Software").join("Opera Stable").join("Extensions"))
                }
                #[cfg(target_os = "linux")]
                {
                    Some(PathBuf::from(&home).join(".config").join("opera").join("Extensions"))
                }
                #[cfg(target_os = "macos")]
                {
                    Some(PathBuf::from(&home).join("Library").join("Application Support").join("com.operasoftware.Opera").join("Extensions"))
                }
                #[cfg(not(any(target_os = "windows", target_os = "linux", target_os = "macos")))]
                {
                    None
    ...
    async fn scan_chrome_extensions(&self) -> Result<Vec<BrowserExtension>> {
        let ext_dir = self.get_extension_dir(&BrowserType::Chrome)
            .ok_or_else(|| GreekError::SystemError("Chrome extensions directory not found".to_string()))?;
        
        self.scan_chromium_extensions(&ext_dir, BrowserType::Chrome).await
    }

    /// Scan Edge extensions (Chromium-based)
    async fn scan_edge_extensions(&self) -> Result<Vec<BrowserExtension>> {
        let ext_dir = self.get_extension_dir(&BrowserType::Edge)
            .ok_or_else(|| GreekError::SystemError("Edge extensions directory not found".to_string()))?;
        
        self.scan_chromium_extensions(&ext_dir, BrowserType::Edge).await
    }

    /// Scan Opera extensions (Chromium-based)
    async fn scan_opera_extensions(&self) -> Result<Vec<BrowserExtension>> {
        let ext_dir = self.get_extension_dir(&BrowserType::Opera)
            .ok_or_else(|| GreekError::SystemError("Opera extensions directory not found".to_string()))?;
        
        self.scan_chromium_extensions(&ext_dir, BrowserType::Opera).await
    }

    /// Scan Chromium-based browser extensions
    async fn scan_chromium_extensions(&self, ext_dir: &PathBuf, browser: BrowserType) -> Result<Vec<BrowserExtension>> {
        let mut extensions = Vec::new();
        ...
        let entries = std::fs::read_dir(ext_dir)
            .map_err(|e| GreekError::ScanError(format!("Failed to read extensions directory: {}", e)))?;
                ...
                if let Ok(version_dirs) = std::fs::read_dir(&path) {
                    for version_dir in version_dirs.flatten() {
                        ...
                        if version_path.is_dir() {
                            if let Some(ext) = self.parse_chromium_extension(&version_path, &browser).await {
                                extensions.push(ext);
                            }
                        }
                    }
    ...
    async fn parse_chromium_extension(&self, path: &PathBuf, browser: &BrowserType) -> Option<BrowserExtension> {
    ...
    async fn scan_firefox_extensions(&self) -> Result<Vec<BrowserExtension>> {
        let ext_dir = self.get_extension_dir(&BrowserType::Firefox)
            .ok_or_else(|| GreekError::SystemError("Firefox extensions directory not found".to_string()))?;
        ...
        let entries = std::fs::read_dir(&ext_dir)
            .map_err(|e| GreekError::ScanError(format!("Failed to read Firefox profiles: {}", e)))?;
        ...
        for entry in entries.flatten() {
            ...
            if profile_path.is_dir() {
                ...
                if extensions_json.exists() {
                    if let Ok(profile_exts) = self.parse_firefox_extensions_json(&extensions_json, &profile_path).await {
    ...
    async fn parse_firefox_extensions_json(&self, json_path: &PathBuf, profile_path: &PathBuf) -> Result<Vec<BrowserExtension>> {
        let content = std::fs::read_to_string(json_path)
            .map_err(|e| GreekError::SystemError(format!("Failed to read extensions.json: {}", e)))?;
        
        let data: serde_json::Value = serde_json::from_str(&content)
            .map_err(|e| GreekError::SystemError(format!("Failed to parse extensions.json: {}", e)))?;
    ...
    fn parse_firefox_addon(&self, addon: &serde_json::Value, profile_path: &PathBuf) -> Option<BrowserExtension> {
        ...
        let version = addon.get("version")
            .and_then(|v| v.as_str())
            .unwrap_or("0.0.0")
            .to_string();
        
        let description = addon.get("defaultLocale")
            .and_then(|l| l.get("description"))
            .and_then(|d| d.as_str())
            .map(String::from);
        
        let enabled = addon.get("active")
            .and_then(|a| a.as_bool())
            .unwrap_or(false);
        
        let install_time = addon.get("installDate")
            .and_then(|t| t.as_str())
            .map(String::from);
    ...
    pub async fn find_extensions_for_app(&self, app: &InstalledApp) -> Result<Vec<BrowserExtension>> {
        ...
        let matching: Vec<BrowserExtension> = all_extensions
            ...
            .filter(|ext| {
                ...
                let desc_match = ext.description.as_ref()
                    .map(|d| d.to_lowercase().contains(&app_name_lower))
                    .unwrap_or(false);
                let publisher_match = publisher_lower.as_ref()
                    .map(|p| ext.name.to_lowercase().contains(p) || 
                         ext.description.as_ref().map(|d| d.to_lowercase().contains(p)).unwrap_or(false))
                    .unwrap_or(false);
