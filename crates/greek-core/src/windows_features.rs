...
impl WindowsFeaturesScanner {
    ...
    async fn scan_powershell_features(&self) -> Result<Vec<WindowsFeature>> {
        ...
        let features_json: Vec<serde_json::Value> =
            serde_json::from_str(&json_str).map_err(|e| {
                GreekError::SystemError(format!("Failed to parse features JSON: {}", e))
            })?;
    ...
    async fn scan_capabilities(&self) -> Result<Vec<WindowsFeature>> {
        ...
        let caps_json: Vec<serde_json::Value> = serde_json::from_str(&json_str).map_err(|e| {
            GreekError::SystemError(format!("Failed to parse capabilities JSON: {}", e))
        })?;
        ...
        let matching: Vec<WindowsFeature> = all_features
            ...
            .filter(|feature| {
                ...
                let publisher_match = publisher_lower
                    .as_ref()
                    .map(|p| {
                        feature.name.to_lowercase().contains(p)
...
mod tests {
    ...
    async fn test_scanner_creation() {
        ...
        let _ = scanner.scan_features().await;
    }
