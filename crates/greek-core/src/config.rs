...
impl ConfigManager {
    pub fn new() -> Result<Self> {
        let proj_dirs = ProjectDirs::from("com", "reek", "reek-uninstaller")
            .ok_or_else(|| GreekError::ConfigError("Failed to get project directories".to_string()))?;
    ...
    pub fn save_config(&self, config: &GreekConfig) -> Result<()> {
        ...
        let content = toml::to_string_pretty(config).map_err(|e| GreekError::ConfigError(e.to_string()))?;
        fs::write(&self.config_file, content).map_err(|e| GreekError::ConfigError(e.to_string()))?;
