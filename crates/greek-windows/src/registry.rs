...
use greek_common::{
    AppScanner, InstalledApp, InstallSource, ScanError, RegistryHive, RegistryKey, RegistryValue, RegistryValueType,
...
impl WindowsRegistryScanner {
    ...
    pub fn with_options(scan_64bit: bool, scan_32bit: bool, include_system_components: bool) -> Self {
        Self {
            scan_64bit,
            scan_32bit,
    ...
    fn scan_hive(&self, hive: RegistryHive, is_64bit: bool) -> Result<Vec<InstalledApp>, ScanError> {
        let mut apps = Vec::new();
        
        let uninstall_path = if is_64bit { UNINSTALL_PATH_NATIVE } else { UNINSTALL_PATH_WOW64 };
    ...
    fn parse_registry_entry(
        ...
        let mut app = InstalledApp::new(
            ...
            InstallSource::Registry {
                hive,
                key_path: format!("{}\\{}", if is_64bit { UNINSTALL_PATH_NATIVE } else { UNINSTALL_PATH_WOW64 }, subkey_name),
            },
        );
        
        // Get optional values
        app.publisher = key.get_value::<String, _>("Publisher").ok().map(|p| clean_publisher_name(&p));
        app.version = key.get_value::<String, _>("DisplayVersion").ok();
        app.install_location = key.get_value::<String, _>("InstallLocation").ok().map(PathBuf::from);
        ...
        app.is_system_component = key.get_value::<u32, _>("SystemComponent")
            .ok()
            .map(|v| v == 1)
            .unwrap_or(false);
        ...
        // Add metadata
        app.metadata.insert("registry_key".to_string(), subkey_name.to_string());
        app.metadata.insert("hive".to_string(), hive.as_str().to_string());
    ...
    fn extract_registry_key(&self, key: &RegKey, subkey_name: &str, hive: RegistryHive) -> RegistryKey {
...
impl AppScanner for WindowsRegistryScanner {
    ...
    async fn scan(&self) -> Result<Vec<InstalledApp>, greek_common::GreekError> {
        ...
        let all_apps = tokio::task::spawn_blocking(move || {
            let scanner = WindowsRegistryScanner::with_options(scan_64bit, scan_32bit, include_system);
        ...
        tracing::info!("Windows registry scan completed, total apps: {}", all_apps.len());
